...
var escapePauses = true;

// How far you have to pull an analog stick before it registers as a PICO-8 d-pad direction
  ...
  for (var i = 0; i < gamepads.length; i++) {
    ...
    bitmask |= pressedX ? 32 : 0;
  	...
  	// we allow all controllers to influence it, regardless of number of players
		// we also translate Escape to the pause button, as it's the key most players
		// are used to for pausing/in-game menus
  	pico8_buttons[0] |= (btn(gp,8) || btn(gp,9) || escapePressed) ? 64 : 0;
  }

  requestAnimationFrame(updateGamepads);
}

// Allowing ESC to trigger the cart menu as well as Enter requires a few listeners
if (escapePauses) {
	document.addEventListener('keydown', function(e) { if (e.keyCode == 27) escapePressed = true; }, false);
	document.addEventListener('keyup', function(e) { if (e.keyCode == 27) escapePressed = false; }, false);
}
